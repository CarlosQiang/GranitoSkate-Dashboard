...
// Función actualizada para eliminar un pedido usando la API REST de Shopify
export async function deleteOrder(orderId) {
  try {
    // Asegurarse de que el ID tenga el formato correcto
    const isFullShopifyId = orderId.includes("gid://shopify/Order/")
    const numericId = isFullShopifyId ? orderId.split("/").pop() : orderId

    console.log(`Deleting order with ID: ${numericId}`)

    // Usamos la API REST de Shopify en lugar de GraphQL para cancelar el pedido
    const cancelResponse = await fetch(`/api/shopify/rest/orders/${numericId}/cancel`, {
      method: "POST",
      headers: {
    ...
    if (!cancelResponse.ok) {
      ...
      console.error("Error al cancelar el pedido:", errorData)
      throw new Error(`Error al cancelar el pedido: ${errorData.message || cancelResponse.statusText}`)
    }

    // Después de cancelar, archivamos el pedido
    const closeResponse = await fetch(`/api/shopify/rest/orders/${numericId}/close`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!closeResponse.ok) {
      ...
      console.error("Error al cerrar el pedido:", errorData)
      // No lanzamos error aquí, ya que el pedido ya está cancelado
      console.warn("El pedido fue cancelado pero no pudo ser cerrado")
    }
