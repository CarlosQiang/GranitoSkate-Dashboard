...
export async function GET(request: Request) {
  try {
    ...
    const tablas = ["productos", "promociones", "registro_sincronizacion"]
    const resultados = {}
    ...
    for (const tabla of tablas) {
      try {
        ...
        resultados[tabla] = result.rows[0].exists
      } catch (error) {
    ...
    return NextResponse.json({
      success: true,
      tablas: resultados,
      todasExisten: Object.values(resultados).every((v) => v === true),
    })
  } catch (error) {
    console.error("Error al verificar la base de datos:", error)
    return NextResponse.json({ error: "Error al verificar la base de datos" }, { status: 500 })
  }
}
