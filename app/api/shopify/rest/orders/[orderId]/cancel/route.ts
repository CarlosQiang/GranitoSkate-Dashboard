...
export async function POST(request: NextRequest, { params }: { params: { orderId: string } }) {
  try {
    ...
    const url = `https://${shopifyDomain}/admin/api/2023-10/orders/${orderId}/cancel.json`

    // Preparar los datos para la cancelación
    const cancelData = {
      reason: body.reason || "customer", // customer, inventory, fraud, declined, other
      email: body.email !== false, // Por defecto, enviar email
      restock: body.restock !== false, // Por defecto, restock
    }
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ cancel: cancelData }),
    })
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: "Error al cancelar el pedido", details: errorData },
